...
iedMkr=_this select 1;
detonateIeds=_this select 2;
...

{
	...
	for "_i" from 1 to iedNum do
    {
        ...
        _ied setPosATL(getPosATL _ied select 2+0.5);
        _ied setDir(random 359);
        if (detonateIeds) then
        {
            [_ied]call iedAct;
        }
        else
