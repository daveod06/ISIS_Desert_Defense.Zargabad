...
while {alive _bomber && _runCode == 1} do
{
	...
	if(count _nearUnits != 0) then
	{
		...
		[_explosive,_bomber,_explosive_bag] spawn {_explosive = _this select 0; _bomber = _this select 1; _explosive_bag = _this select 2; waitUntil {!alive _bomber}; deleteVehicle _explosive; deleteVehicle _explosive_bag;};
